...
describe('util', () => {
  ...
  it('.serialize and .deserialize', () => {
    ...
    // d8 = tag, 2a = 42
    expect(serializedObj.toString('hex').match(/d82a/g)).to.have.length(4)
  ...
  it('reject extraneous, but valid CBOR data after initial top-level object', () => {
    expect(() =>
      ...
      dagCBOR.util.deserialize(
        uint8ArrayConcat([serializedObj, new Uint8Array(1)])
      )
    ).to.throw(
      Error,
  ...
  it('serialize str links', () => {
    const objWithLink = {
      foo: new CID(
        'bafyreicaoyussrycqolu4k2iaxleu2uakjlq57tuxq3djxn4wnyfp4yk3y'
      )
    }
    ...
    expect(dagCBOR.util.deserialize(s1)).to.be.eql(objWithLink)
    expect(dagCBOR.util.deserialize(s2)).to.be.eql(objWithLink)
  })
})
